...
const AdminHomeSector = () => {
    ...
    const addUser = async (e) => {
        ...
        setError(null);
        try {
            await axios.post("https://jsd5-mock-backend.onrender.com/members", { name, lastName, position });
            fetchUsers();
            setName('');
            setLastName('');
            setPosition('');
    ...
    const deleteUser = async (id) => {
        ...
        setError(null);
        try {
            await axios.delete(`https://jsd5-mock-backend.onrender.com/users/${id}`);
            fetchUsers();
        } catch (error) {
    ...
    return (
        <div>
            ...
            <div>
                <form onSubmit={addUser} className='flex justify-center gap-10'>
                    <input type="text" placeholder="Name" value={name} onChange={(e) => setName(e.target.value)} className='bg-slate-50 border border-slate-400 rounded-md p-2' />
                    <input type="text" placeholder="Last Name" value={lastName} onChange={(e) => setLastName(e.target.value)} className='bg-slate-50 border border-slate-400 rounded-md p-2' />
                    <input type="text" placeholder="Position" value={position} onChange={(e) => setPosition(e.target.value)} className='bg-slate-50 border border-slate-400 rounded-md p-2' />
                    <button type='submit' disabled={loading} className='bg-sky-500 p-2 rounded-md text-white'>Save</button>
                    {loading && <p>Loading...</p>}
